...
pragma and can be placed anywhere within the contract, though it is generally considered best practice to specify it at the top of the document. For single-line comments, we need to use //. For multiline comments, we need to use /* … */. 
Solidity comments can also be written in NatSpec format. Developers use it for documenting return variables, functions, and so on. To define a single-line comment in this format, we need to use /// for multiline, we need to use /** … */. 
...
1. One of the other functions match the function identifier 
...
Compile time or runtime errors can occur. Error handling can be done using asser, require, revert, try-catch and custom errors.
...
`External contract` is deployed. If there is an error catch blocks will be executed. The custom error is defined as Logic in the below snippet. It is thrown when the input/argument of the function `testCustom_error` is greater than 10. 
```solidity
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.5;
