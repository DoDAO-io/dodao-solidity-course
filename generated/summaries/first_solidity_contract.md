...
- SPDX License Identifier
  * The source code of a smart contract needs to be open source for it to be trusted and to prevent any legal issues with regards to copyright, machine-readable SPDX license identifiers should be used.
...
- Imports
  * Solidity supports import statements to modularize code similar to those available in JavaScript (from ES6 on).
...
* Not all types of constants and immutables are implemented at this time. The only supported types are strings (only for constants) and value types(booleans, integers, address and enum).
...
* In the code below helper function is defined outside the contract whearas arithmetic function is declared inside the contract.
  ...
  ```
* Functions outside of a contract are also called 'free functions' and their code is included in all contracts that call them.
